...
class Mocha_Helper {
	static collectTests(dir) {
		return new Promise((resolve, reject) => {
			...
			output.debug(`Found ${tests.length} test(s)`);
	...
	static run(testFiles) {
		return new Promise((resolve, reject) => {
			...
			const
				...
				modFile = modParent.parent.filename,
				modRoot = path.dirname(modFile);
			...
			// Create the labels
			const
				outDir = path.join(modRoot, 'Results'),
				outFile = path.join(outDir, `${testTarg}_${time}.xml`);

			// Make sure our result location is defined
