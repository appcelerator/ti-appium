	...
	static build(dir, platform, args) {
		return new Promise((resolve, reject) => {
			...
			prc.on('exit', code => {
				...
				if(platform === 'ios') {
					appPath = path.join(dir, 'build', 'iphone', 'build', 'Products', 'Debug-iphonesimulator', `${appName}.app`)
				} else if(platform === 'android') {
					appPath = path.join(dir, 'build', 'android', 'bin', `${appName}.apk`);
				}

				(code === 0 && appPath) ? output.finish(resolve, appPath): reject('Failed on application build');
			});
		});
	}
