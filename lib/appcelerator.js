	...
	static build(dir, platform, args) {
		return new Promise((resolve, reject) => {
			...
			prc.stderr.on('data', data => {
				...
				// If statement is there so that [WARN] flags are ignored on stderr
				if(data.toString().includes('[ERROR]')) return reject(data.toString().replace('[ERROR] ', ''));
			});
	...
	static runner(args, matcher) {
		return new Promise((resolve, reject) => {
			...
			prc.stdout.on('data', data => {
				...
				if((typeof matcher) === 'function' && matcher(data.toString())) return resolve(data.toString());
			});
