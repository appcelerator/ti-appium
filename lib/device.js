...
class Device_Helper {
	/*****************************************************************************
	 * Pass off to the relevant kill command dependant on passed platform
	 *
	 * @param {String} platform - The category of virtual device to be shut down
	 ****************************************************************************/
	static async killDevice(platform) {
		if(platform === 'iOS') {
			await killSim();
	...
	static launchEmu(devName, platform) {
		return new Promise(resolve => {
			output.step(`Launching Android device '${devName}'`);
	...
	static launchGeny(devName, platform) {
		return new Promise(resolve => {
			output.step(`Booting Genymotion Emulator '${devName}'`);
 ...
 ******************************************************************************/
async function killEmu() {
	if(process.platform === 'darwin') {
		await childProcess.execSync('pkill -9 qemu-system-i386');
		await childProcess.execSync('pkill -9 player');
	}
	if(process.platform === 'win32') {
		await childProcess.execSync('tskill qemu-system-i386');
		await childProcess.execSync('tskill player');
	}
}
