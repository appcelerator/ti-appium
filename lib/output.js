...
class Output_Helper {
	...
	static step(message) {
		...
		if(process.env.logging === 'debug') message = `${message}\n`;

		if(process.env.logging) process.stdout.write(message);
	}
	...
	static finish(done, value) {
		if(process.env.logging === 'basic') process.stdout.write(`${Green}Done${Reset}\n`);
	...
	static skip(done, value) {
		if(process.env.logging === 'basic') process.stdout.write(`${Yellow}Skipping${Reset}\n`);
	...
	static error(message) {
		...
		if(process.env.logging === 'basic') message = `\n${message}`;
	...
	static banner(message) {
		if(process.env.logging) process.stdout.write('\n-------------------------------------------------------\n');
		if(process.env.logging) process.stdout.write(`${Green}[INFO]${Reset} ${message}\n`);
		if(process.env.logging) process.stdout.write('-------------------------------------------------------\n');
	}
	...
	static debug(message) {
		...
		if(process.env.logging === 'debug') process.stdout.write(message);
	}
}
